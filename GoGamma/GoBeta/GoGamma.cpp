...
int CreateNode(int BeforePlace,int DameFlag){
	NODE *PointerNode;
	int x, y, Place;
	double bonus;
...
int SelectbestUCB(int NextNode,int DameFlag){
	...
	int BreakNum;	//printf("ChildGamesSum:%d",PointerNode->ChildGamesSum);
	if (DameFlag==ON){
		BreakNum=(int)(1.0 + log(PointerNode->ChildGamesSum + 1.0) / log(1.5));
	}
	else{
		BreakNum = (int)(1.0 + log(PointerNode->ChildGamesSum + 1.0) / log(1.2));
	}
			...
			if (FlagBoard[NowPlace + dir4[i]] == OFF && board[NowPlace + dir4[i]] == 0){
				...
				for (int j = 0; j<4; j++) {
					if (board[NowPlace + dir4[i] + dir4[j]] == color) {
						flagColor = ON;
						LiveDieFlag.push_back(FlagBoard[NowPlace + dir4[i] + dir4[j]]);
					}
...
int search_uct(int color, int NextNode,int DameFlag){
	NODE *PointerNode = &NodeArray[NextNode];
	CHILD *BestChild = NULL;
	int SelectChildIndex;
	int place, err, win;
	while (1){
		SelectChildIndex = SelectbestUCB(NextNode,DameFlag);
	...
	if (BestChild->games == 0 || depth == DEPTH_MAX || (BestChild->place == 0 && depth >= 2 && path[depth - 2] == 0)){
		...
		if (BestChild->NextNode == NODE_EMPTY) {
			BestChild->NextNode = CreateNode(BestChild->place,DameFlag);
		}
		win = -search_uct(FlipColor(color), BestChild->NextNode,DameFlag);
	}
...
const int UctLoop = 500;
int get_best_uct(int color,int DameFlag){
	int  next;
	NODE *PointerNode;
	int BestPlace = 0;
	...
	if (YoseFlag==OFF&&DameFlag==OFF){
		if (moves > 50 && PointerNode->child[BestChildNum].rate < 0.1 && color == 1 || moves>50 && PointerNode->child[BestChildNum].rate > -0.1 && color == 2){
			YoseFlag = ON;
			BestPlace = MontecalroDameSearch(color);
		}
	}
	return BestPlace;
}
...
int MontecalroDameSearch(int color){
	int DameFlag = ON;
	MakeDamePlace(color);
	return get_best_uct(color, DameFlag);
}
