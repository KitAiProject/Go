...
{
    int i, x, y;
    for (i = 1; i <= BOARD_MAX; i++) board[i] = 3;
    for (y = 0; y <= B_SIZE; y++) for (x = 1; x <= B_SIZE; x++) board[get_z(x, y)] = 0;
    MoveNum = 0;
    ko_z = 0;
}
...
void print_board()
{
    ...
    for (x = 1; x <= B_SIZE; x++) prt("%c", 'A' + x + (x>7));
    prt("\n");
    for (y = 1; y <= B_SIZE; y++) {
        //  prt("%2d ",y+1);
        prt("%2d ", B_SIZE - y);
        for (x = 1; x <= B_SIZE; x++) {
            prt("%s", str[board[get_z(x, y)]]);
        }
        if (y == 4) prt("  ko_z=%d,moves=%d", get81(ko_z), MoveNum);
        prt("\n");
    }
}
