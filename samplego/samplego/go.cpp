...
int create_node(int prev_z)
{
	...
	//prt("node_num:%d\n",node_num);
	pN = &node[node_num];//node_num‚ÍV‚µ‚­ƒm[ƒh‚ðì‚é‚Æ‚«‚ÉŽg‚¤B
	//ÅŒã‚É{‚P‚µ‚ÄŽŸ‚Ìƒm[ƒh‚ð‚·‚®‚Éì‚ê‚é‚æ‚¤‚É‚µ‚Ä‚¢‚éB
	pN->child_num = 0;
...
int search_uct(int color, int node_n)//node_n‚Í
{
	NODE *pN = &node[node_n];
	CHILD *c = NULL;
	...
	if (c->games <= 0 || depth == D_MAX || (c->z == 0 && depth >= 2 && path[depth - 2] == 0)) {
		...
		if (c->next == NODE_EMPTY){
			...
			prt("search_uct create_node-> node_num:%d\n",node_num);
		}
