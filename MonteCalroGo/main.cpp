...
typedef struct {
    ...
    double rate;  // winrate
    int next;     // next node それぞれのchildが、次のノードを持っている。
...
int search_uct(int color, int node_n)//node_nはｃ->nextのこと つまり次のノード番号
{//serch_uctで再帰をするたびに
    NODE *pN = &node[node_n];
    CHILD *c = NULL;
    int select, z, err, win;
    //prt("node_n(search_uct):%d\n",node_n);
    ...
    path[depth++] = c->z;
    //prt("path:%d depth:%d",path[depth],depth);
    // playout in first time. <= 10 can reduce node                   パス↓
    if ( c->games <= 0 || depth == D_MAX || (c->z == 0 && depth>=2 && path[depth-2]==0) ) {//depthが勝手にインクリメントされてしまうので1手前は-2
        win = -playout(flip_color(color));
        //prt("playout  PointerChild:%d depth:%d\n",c->z,depth,path[depth-2]);
    } else {
        if ( c->next == NODE_EMPTY ) c->next = create_node(c->z);//cはchildのこと　ノード番号が１つ増える。
        win = -search_uct(flip_color(color), c->next);//c->nextが再帰でnode_nになる！　childのnextが指す次のノードで再帰をする
    }

    // update winrate
    //winが求められたので、選ばれたchildのレートを更新する。（選ばれたchildでシミュレーションした結果winが求められた。）
    //再帰が終わってwinが求まったらその経路（木）のレートを更新して　return する　->また再帰
    c->rate = (c->rate * c->games + win) / (c->games + 1);//ノードのrateとgamesを更新　１回目はplayout１回の情報だけ更新　ノードを降りたら戻るときにすべてを更新
    c->games++;
    pN->child_games_sum++;
    //select_best_ucbで、childの勝率などを使うため、更新しておく
    return win;
}
